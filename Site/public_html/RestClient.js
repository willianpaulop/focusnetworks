...
// Model for Tbparticipantes entity
    models.Tbparticipantes = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/AmigoSecreto/webresources/com.mycompany.amigosecreto.tbparticipantes/",
        idAttribute: 'id',
        defaults: {
            name: "",
            email: ""
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8084/AmigoSecreto/webresources/com.mycompany.amigosecreto.tbparticipantes/';
            }
    ...
    // Collection class for Tbparticipantes entities
    models.TbparticipantesCollection = Backbone.Collection.extend({
        model: models.Tbparticipantes,
        url: "http://localhost:8084/AmigoSecreto/webresources/com.mycompany.amigosecreto.tbparticipantes/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.TbparticipantesCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-tbparticipantes-list-itemis template identifier for item
                        templateName: '#tpl-tbparticipantes-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.tbparticipantes = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.tbparticipantes,
                    // tpl-tbparticipantes-details is template identifier for chosen model element
                    templateName: '#tpl-tbparticipantes-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Tbparticipantes();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-tbparticipantes-details is a template identifier for chosen model element
                templateName: '#tpl-tbparticipantes-details',
